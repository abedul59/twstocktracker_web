<template>
  <div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3 flex-wrap gap-3">
      ...
      <div class="d-flex align-items-center gap-3 flex-wrap">
        ...
        <div class="d-flex align-items-center gap-2" v-if="availableDates.length > 0">
          ...
          <select class="form-select w-auto fw-bold shadow-sm" v-model="selectedDate">
    ...
    <div v-if="availableDates.length === 0 && !isScraping" class="alert alert-warning text-center shadow-sm border-0">
    ...
    <div v-if="statsData.length > 0" class="data-section card shadow-sm border-0 overflow-hidden">
      ...
      <div v-if="mainTab === '類股'" class="p-3">
        <div class="d-flex justify-content-center gap-2 mb-3">
          <button class="btn btn-sm fw-bold" :class="marketTab === '上市' ? 'btn-danger' : 'btn-outline-danger'" @click="marketTab = '上市'">上市類股</button>
          <button class="btn btn-sm fw-bold" :class="marketTab === '上櫃' ? 'btn-primary' : 'btn-outline-primary'" @click="marketTab = '上櫃'">上櫃類股</button>
          <button class="btn btn-sm fw-bold" :class="marketTab === '興櫃' ? 'btn-success' : 'btn-outline-success'" @click="marketTab = '興櫃'">興櫃類股</button>
        </div>
          ...
          <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
              ...
              <tr v-if="filteredStats.length === 0">
                <td colspan="5" class="text-center py-4 text-muted">此市場當日無資料</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
              <tr v-for="stock in top30Data" :key="stock.id">
                <td><span class="badge bg-dark fs-6">#{{ stock.rank }}</span></td>
                ...
                <td class="text-end fw-bold text-danger">{{ stock.total_value }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const triggerCloudScrape = async () => {
  ...
  try {
    // 呼叫我們在 Nuxt Server 端寫好的 trigger.post.ts
    const response = await $fetch('/api/trigger', { method: 'POST' })
    scrapeStatus.value = `✅ ${response.message}`
    
    // 爬取完畢後，重新掃描資料庫日期，並自動切換畫面到最新的一天
